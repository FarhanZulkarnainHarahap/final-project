...
export async function sendVerificationEmail(req, res) {
    ...
    try {
        ...
        const templateSource = await fs.readFile("src/templates/verify-email.hbs", "utf-8");
...
export async function confirmEmail(req, res) {
    ...
    try {
        ...
        if (user.isVerified) {
            res.redirect("http://localhost:3000/dashboard/user/profile?status=already-verified");
            return;
        }
        await prisma.user.update({
            where: { id: user.id },
...
export async function VerifySuccess(req, res) {
    ...
    try {
        ...
        if (user.isVerified) {
            ...
            const templateSource = await fs.readFile("src/templates/verify-complated.hbs", "utf-8");
            const compiledTemplate = handlebars.compile(templateSource);
