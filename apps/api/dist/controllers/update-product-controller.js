...
export async function updateProduct(req, res) {
    try {
        const { id } = req.params;
        const { name, description, price, weight, categoryIds, storeStocks } = req.body;
        const user = req.user;
        ...
        if (user.role === "STORE_ADMIN") {
            if (storeStocks) {
                ...
                const invalidStores = requestedStoreIds.filter((storeId) => !userStoreIds.includes(storeId));
                if (invalidStores.length > 0) {
                    console.log("Invalid stores:", invalidStores);
                    res.status(403).json({
                        message: "You don't have permission to update stock for these stores",
                        invalidStores,
                    });
                    return;
                }
                    ...
                    if (difference !== 0) {
                        ...
                        await tx.inventoryJournal.create({
                            data: {
                                ...
                                productId: id,
                                quantity: difference > 0 ? quantity.toString() : (-quantity).toString(),
                                weight: currentProduct.weight,
                                action: action, // InventoryAction type
                                userId: user.id,
                            },
                        });
                        console.log(`Journal created: ${action} ${quantity} units for store ${item.storeId}`);
                    }
...
async function handleImageUpdates(productId, files) {
    // Update preview image
    if (files?.imagePreview?.[0]) {
        await prisma.image.deleteMany({
            where: { previewProductId: productId },
        });
        const result = await cloudinary.uploader.upload(files.imagePreview[0].path, {
    ...
    if (files?.imageContent?.[0]) {
        ...
        const result = await cloudinary.uploader.upload(files.imageContent[0].path, {
            folder: "final-project/products",
        });
        await prisma.image.create({
            data: {
