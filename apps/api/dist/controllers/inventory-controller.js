...
// Helper functions
const buildWhereClause = async (user, additionalFilters = {}) => {
...
const updateStockOperation = async (storeId, productId, quantity, action, user, weight, isCreate = false) => {
...
/*                             GET INVENTORY DATA                             */
/* -------------------------------------------------------------------------- */
export async function getInventoryData(req, res) {
    try {
        const user = req.user;
        if (!user) {
...
/* -------------------------------------------------------------------------- */
export async function updateStockWithJournal(req, res) {
    try {
        const user = req.user;
        if (!user) {
            res.status(401).json({ message: "Unauthorized" });
        ...
        const validActions = ["RESTOCK", "SALE", "ADD"];
        ...
        await updateStockOperation(storeId, productId, parseInt(quantity), action, user, product.weight);
...
export async function createStockEntry(req, res) {
    try {
        ...
        await updateStockOperation(storeId, productId, stockNum, "ADD", user, product.weight, true);
...
export async function getInventoryHistory(req, res) {
    try {
        ...
        const { storeId, productId, action, startDate, endDate, page, limit } = req.query;
        const { pageNum, limitNum, skip } = getPaginationData(page, limit);
        // Build filters
        let whereClause = {};
