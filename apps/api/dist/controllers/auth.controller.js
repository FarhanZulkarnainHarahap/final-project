...
export async function register(req, res) {
    try {
        const { email, firstName, lastName, username, password, phoneNumber } = registerSchema.parse(req.body);
        // Hashing password
        ...
        const verificationToken = jwt.sign({ email: newUser.email }, process.env.JWT_SECRET, { expiresIn: "1d" } // Token valid for 1 day
        );
        ...
        const templateSource = await fs.readFile("src/templates/registration-welcoming-template.hbs", "utf-8" // encoding as the third parameter
        );
...
export async function verifyEmail(req, res) {
    ...
    try {
        ...
        const templateSource = await fs.readFile("src/templates/verification-complated.hbs", "utf-8" // encoding as the third parameter
        );
        const compiledTemplate = handlebars.compile(templateSource.toString());
        const htmlTemplate = compiledTemplate({
            currentYear: new Date().getFullYear(),
...
export async function VerifySuccess(req, res) {
